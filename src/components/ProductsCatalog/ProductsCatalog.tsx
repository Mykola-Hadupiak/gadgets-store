...
export const ProductsCatalog: React.FC<Props> = ({ products, name }) => {
  ...
  const correctPerPage = () => {
    return perPage === 'all' ? products.length : +perPage;
  };
  ...
  const phonesWithQuery = products.filter(product =>
    product.name.toLowerCase().includes(query.toLowerCase().trim()),
  );

  const phonesToRender = sortProducts(sortBy, products).slice(from, to);
      ...
      {!query ? (
        <>
          ...
          <div className="products-catalog__top">
            <h1 className="products-catalog__title title">{name}</h1>
          ...
          <div className="products-catalog__filter">
            ...
            <div className="products-catalog__sort-by">
              <p className="products-catalog__selector-name">Items on page</p>
